defmodule SymphonyElixir.WorkspaceAndConfigTest do
  ...
  test "config reads defaults for optional settings" do
    ...
    assert Config.claude_command() == "claude --output-format stream-json"

    assert Config.claude_approval_policy() == %{
             "reject" => %{
    ...
    assert Config.claude_thread_sandbox() == "workspace-write"
    ...
    assert Config.claude_read_timeout_ms() == 5_000
    assert Config.claude_stall_timeout_ms() == 300_000

    write_workflow_file!(Workflow.workflow_file_path(), claude_command: "claude --output-format stream-json --model claude-opus-4")
    assert Config.claude_command() == "claude --output-format stream-json --model claude-opus-4"

    write_workflow_file!(Workflow.workflow_file_path(),
      claude_approval_policy: "on-request",
      claude_thread_sandbox: "workspace-write",
      claude_turn_sandbox_policy: %{type: "workspaceWrite", writableRoots: ["/tmp/workspace", "/tmp/cache"]}
    )

    assert Config.claude_approval_policy() == "on-request"
    assert Config.claude_thread_sandbox() == "workspace-write"
    ...
    assert {:error, {:invalid_claude_approval_policy, ""}} = Config.validate!()

    write_workflow_file!(Workflow.workflow_file_path(), claude_thread_sandbox: "")
    assert Config.claude_thread_sandbox() == "workspace-write"
    assert {:error, {:invalid_claude_thread_sandbox, ""}} = Config.validate!()
    ...
    assert {:error, {:invalid_claude_turn_sandbox_policy, {:unsupported_value, "bad"}}} =
             Config.validate!()
    ...
    assert Config.claude_approval_policy() == "future-policy"
    assert Config.claude_thread_sandbox() == "future-sandbox"
    ...
    write_workflow_file!(Workflow.workflow_file_path(), claude_command: "claude --output-format stream-json")
    assert Config.claude_command() == "claude --output-format stream-json"
  end
  ...
  test "config resolves $VAR references for env-backed secret and path values" do
    ...
    assert Config.claude_command() == "#{claude_bin} app-server"
  end
