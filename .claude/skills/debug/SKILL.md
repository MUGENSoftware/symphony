...
- Primary runtime log: `log/symphony.log`
  ...
  - Includes orchestrator, agent runner, and Claude Code CLI lifecycle logs.
...
4. Trace that `session_id` across start, stream, completion/failure, and stall
   handling logs.
5. Decide class of failure: timeout/stall, CLI startup failure, turn failure,
   or orchestrator retry loop.
...
3. Classify the problem:
    ...
    - CLI startup: `Claude session failed ...`.
